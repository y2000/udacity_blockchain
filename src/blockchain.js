...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            // return message in this format: <WALLET_ADDRESS>:${new Date().getTime().toString().slice(0,-3)}:starRegistry
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            let registered_time = parseInt(message.split(':')[1]);
            console.debug(`Message registered time ${registered_time}`);
            let current_time = parseInt(Date.now().toString().slice(0, -3));
            if (current_time - registered_time <= 5*60){    // within 5 minutes
            ...
            Promise.all(
                self.chain.forEach(b => {
                    b.validate().then(verified => {
                        ...
                        if (!verified){
                            console.warn(`Invalid block at ${b.height}...`);
